const _fetchGeoNames = async (zip = '11230') => {
  const url = `http://localhost:7000/geoNames?zip=${zip}`;
...
const _fetchDarkSky = async (lat, long, time) => {
  const url2 = `http://localhost:7000/darkSky?latitude=${lat}&longitude=${long}&time=${time}`;
...
const _fetchPixabay = async image => {
  const url3 = `http://localhost:7000/pixabay?image=${image}`;
